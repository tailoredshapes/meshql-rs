...
The reason is worth understanding rather than treating as a quirk. A poll-diff tail source detects change by comparing payload hashes, so it cannot see a *token-only* ACL change: it retains the record's old authorization mark until the next payload edit or delete. Filtering a notification through a stale token set leaks only the fact that *something* changed. Filtering a **payload** through it leaks the record itself, to someone who should no longer see it. The server's type system makes the unsafe pairing unrepresentable, so you will never receive an unsound payload — but knowing this explains why the same client code sees payloads on one entity and bare notifications on another, and why the answer is "refetch through the graphlette," not "ask for payloads to be turned on."
