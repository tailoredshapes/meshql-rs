...
impl Repository for SqliteRepository {
    ...
    async fn list(&self, _tokens: &[String]) -> Result<Vec<Envelope>> {
        let rows = sqlx::query(
            "SELECT id, created_at_ms, deleted, authorized_tokens, payload FROM envelopes e1
             WHERE created_at_ms = (SELECT MAX(created_at_ms) FROM envelopes e2 WHERE e2.id = e1.id)
             AND deleted = 0",
        )
        .fetch_all(&self.pool)
        .await
