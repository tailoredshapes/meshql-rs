//! MongoDB-backed egg-economy performance server.
//! Starts the full 13-entity server on the configured port for k6 benchmarking.
//!
//! Usage: cargo run -p meshql-mongo --release --features perf --bin perf_server
//!
//! Environment variables:
//!   PORT      - listen port (default 5088)
//!   MONGO_URI - MongoDB connection string (default mongodb://127.0.0.1:27017)
//!   DB_NAME   - database name (default meshql_perf)
...
async fn make_entity(
    ...
    db: &str,
    collection: &str,
    auth: Arc<dyn meshql_core::Auth>,
) -> Entity {
    let repo = Arc::new(
        MongoRepository::new(uri, db, collection, Arc::clone(&auth))
            .await
            .unwrap(),
    );
    let searcher: Arc<dyn meshql_core::Searcher> =
        Arc::new(MongoSearcher::new(uri, db, collection, auth).await.unwrap());
...
async fn main() -> anyhow::Result<()> {
    ...
    // 3 MongoDB shards — same layout as Java egg-economy
    // Default URIs use directConnection=true for host-mode (RS topology uses Docker-internal
    // hostnames that don't resolve from the host). Write concerns are still honoured.
    // Override with ACTORS_MONGO_URI etc. when running inside Docker compose.
    let actors_uri = std::env::var("ACTORS_MONGO_URI")
        .unwrap_or_else(|_| "mongodb://127.0.0.1:27017/?directConnection=true".into());
    let events_uri = std::env::var("EVENTS_MONGO_URI")
        .unwrap_or_else(|_| "mongodb://127.0.0.1:27018/?directConnection=true".into());
    let projections_uri = std::env::var("PROJECTIONS_MONGO_URI")
        .unwrap_or_else(|_| "mongodb://127.0.0.1:27019/?directConnection=true".into());

    let db = std::env::var("DB_NAME").unwrap_or_else(|_| "eggs_development".into());

    let auth: Arc<dyn meshql_core::Auth> = Arc::new(NoAuth);

    // Shard 1: Actors (farms, coops, hens, containers, consumers)
    let farm = make_entity(&actors_uri, &db, "farm", Arc::clone(&auth)).await;
    let coop = make_entity(&actors_uri, &db, "coop", Arc::clone(&auth)).await;
    let hen = make_entity(&actors_uri, &db, "hen", Arc::clone(&auth)).await;
    let container = make_entity(&actors_uri, &db, "container", Arc::clone(&auth)).await;
    let consumer = make_entity(&actors_uri, &db, "consumer", Arc::clone(&auth)).await;

    // Shard 2: Events (lay_report, storage_deposit, storage_withdrawal, container_transfer, consumption_report)
    let lay_report = make_entity(&events_uri, &db, "lay_report", Arc::clone(&auth)).await;
    let storage_deposit = make_entity(&events_uri, &db, "storage_deposit", Arc::clone(&auth)).await;
    let storage_withdrawal =
        make_entity(&events_uri, &db, "storage_withdrawal", Arc::clone(&auth)).await;
    let container_transfer =
        make_entity(&events_uri, &db, "container_transfer", Arc::clone(&auth)).await;
    let consumption_report =
        make_entity(&events_uri, &db, "consumption_report", Arc::clone(&auth)).await;

    // Shard 3: Projections (container_inventory, hen_productivity, farm_output)
    let container_inventory = make_entity(
        &projections_uri,
        &db,
        "container_inventory",
        Arc::clone(&auth),
    ...
    .await;
    let hen_productivity =
        make_entity(&projections_uri, &db, "hen_productivity", Arc::clone(&auth)).await;
    let farm_output = make_entity(&projections_uri, &db, "farm_output", Arc::clone(&auth)).await;

    // Root configs (same as SQLite perf_server)
