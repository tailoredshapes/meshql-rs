...
async fn build_farm_server(url: &str) -> String {
    ...
    let farm_repo = Arc::new(
        MysqlRepository::new_with_table(url, &farm_table)
            .await
            .unwrap(),
    );
    let coop_repo = Arc::new(
        MysqlRepository::new_with_table(url, &coop_table)
            .await
            .unwrap(),
    );
    let hen_repo = Arc::new(
        MysqlRepository::new_with_table(url, &hen_table)
            .await
            .unwrap(),
    );

    let farm_searcher: Arc<dyn meshql_core::Searcher> = Arc::new(
        MysqlSearcher::new_with_table(url, &farm_table)
            .await
            .unwrap(),
    );
    let coop_searcher: Arc<dyn meshql_core::Searcher> = Arc::new(
        MysqlSearcher::new_with_table(url, &coop_table)
            .await
            .unwrap(),
    );
    let hen_searcher: Arc<dyn meshql_core::Searcher> = Arc::new(
        MysqlSearcher::new_with_table(url, &hen_table)
            .await
            .unwrap(),
    );
