...
use meshql_core::{
    Envelope, MeshqlError, Operation, Repository, Result, RootConfig, Session, SystemSession,
};
...
impl Repository for DynamoRepository {
    ...
    async fn remove(&self, id: &str, session: &dyn Session) -> Result<bool> {
        ...
        match self.read(id, &SystemSession, None).await? {
            None => Ok(false),
            Some(env) if !session.is_authorized(Operation::Remove, &env) => Ok(false),
            Some(env) => {
                // The tombstone carries the *resolved version's* mark, not one
                // the caller's session would stamp. Re-stamping would change
                // visibility as a side effect of a delete, and
                ...
                self.create(tombstone, &SystemSession).await?;
                Ok(true)
            }
        }
    }
    ...
    async fn read_many(&self, ids: &[String], session: &dyn Session) -> Result<Vec<Envelope>> {
        ...
        let reads = ids.iter().map(|id| self.read(id, session, None));
    ...
    async fn remove_many(
        ...
        for id in ids {
            results.insert(id.clone(), self.remove(id, session).await?);
        }
        Ok(results)
    }
