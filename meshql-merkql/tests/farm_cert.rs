...
type Coop { id: ID name: String }
type Query {
    getHen(id: ID, at: Float): Hen
    getHens(name: String, at: Float): [Hen]
    getHensByCoop(id: ID, at: Float): [Hen]
}
";
...
async fn build_farm_server() -> String {
    ...
    let farm_topic = format!("farms_{}", &suffix[..8]);
    let coop_topic = format!("coops_{}", &suffix[..8]);
    let hen_topic = format!("hens_{}", &suffix[..8]);

    let farm_repo = Arc::new(MerkqlRepository::new(broker.clone(), &farm_topic));
    let coop_repo = Arc::new(MerkqlRepository::new(broker.clone(), &coop_topic));
    let hen_repo = Arc::new(MerkqlRepository::new(broker.clone(), &hen_topic));

    let farm_searcher: Arc<dyn meshql_core::Searcher> =
        Arc::new(MerkqlSearcher::new(broker.clone(), &farm_topic));
    let coop_searcher: Arc<dyn meshql_core::Searcher> =
        Arc::new(MerkqlSearcher::new(broker.clone(), &coop_topic));
    let hen_searcher: Arc<dyn meshql_core::Searcher> =
        Arc::new(MerkqlSearcher::new(broker.clone(), &hen_topic));
