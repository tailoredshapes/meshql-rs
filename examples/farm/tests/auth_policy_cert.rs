...
async fn actors_get_full_crud_by_default() {
    ...
    let delete = client
        .delete(format!("{addr}/farm/api/{id}"))
        .send()
        .await
        .unwrap();
...
async fn lay_report_is_create_only() {
    ...
    let created: serde_json::Value = client
        ...
        .json(
            &serde_json::json!({"henId": "hen-1", "eggs": 2, "timeOfDay": "2026-07-22T08:00:00Z"}),
        )
        .send()
        .await
        .unwrap()
    ...
    let delete = client
        .delete(format!("{addr}/lay_report/api/{id}"))
        .send()
        .await
        .unwrap();
...
async fn hen_productivity_denies_default_caller_every_verb() {
    ...
    assert_eq!(
        create.status(),
        403,
        "hen_productivity create must be denied to the default caller"
    );
}
...
async fn worker_role_can_create_and_update_hen_productivity() {
    ...
    let auth = CasbinAuth::from_strings(model, policy, NoAuth)
        .await
        .unwrap();
    ...
    assert!(
        !auth.authorize_action(&[], "create"),
        "default caller has no role in this policy"
    );
}
