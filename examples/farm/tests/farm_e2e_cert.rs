...
//! duplicate — round-trips data through REST writes and GraphQL reads for
//! the plain (non-event-sourced) farm/coop/hen entities.
//!
//! This closes a real gap: `meshql-mongo/tests/farm_cert.rs` on `main` (and
//! the shared `farm.feature` it runs) already prove the *pattern* is
//! correct with its own copy of the RootConfig, but never exercise
//! `examples/farm`'s own code, and never call the plain
//! `getFarms`/`getCoops`/`getHens` vector queries directly (only `getFarm`
//! and the `*ByFarm`/`*ByCoop` federation resolver targets, via different
//! query templates). This branch's own `getFarms`/`getCoops`/`getHens`/
//! `getCoopsByFarm`/`getHensByCoop` templates filtered on top-level fields
//! (`{"name": "{{name}}"}`) instead of `payload`-nested ones
//! (`{"payload.name": "{{name}}"}`), so every payload document — always
//! stored as `{payload: {name, ...}}` — silently matched nothing.
...
async fn spawn_farm() -> String {
    ...
    // (matches the convention already used in auth_policy_cert.rs).
