...
async fn main() -> Result<(), lambda_http::Error> {
    let ksql_config = KsqlConfig::from_env()
        .map_err(|e| lambda_http::Error::from(format!("missing Confluent env vars: {e}")))?;
    let client = Arc::new(ConfluentClient::new(&ksql_config));

    let auth: Arc<dyn meshql_core::Auth> = Arc::new(NoAuth);
    let _ = auth; // NoAuth used implicitly

    // Helper to create repo + searcher + initialize DDL
    macro_rules! entity {
        ($name:expr) => {{
            let repo = Arc::new(KsqlRepository::new(client.clone(), $name, &ksql_config));
            let searcher: Arc<dyn meshql_core::Searcher> =
                Arc::new(KsqlSearcher::new(client.clone(), $name));
            if ksql_config.auto_create_ddl {
                if let Err(e) = repo.initialize().await {
                    eprintln!(
                        "WARN: DDL init for {} failed (will retry on use): {e}",
                        $name
                    );
                }
            }
    ...
    // Actors
    let (farm_repo, farm_searcher) = entity!("farm");
    let (coop_repo, coop_searcher) = entity!("coop");
    let (hen_repo, hen_searcher) = entity!("hen");
    let (container_repo, container_searcher) = entity!("container");
    let (consumer_repo, consumer_searcher) = entity!("consumer");

    // Events
    let (lay_report_repo, lay_report_searcher) = entity!("lay_report");
    let (storage_deposit_repo, storage_deposit_searcher) = entity!("storage_deposit");
    let (storage_withdrawal_repo, storage_withdrawal_searcher) = entity!("storage_withdrawal");
    let (container_transfer_repo, container_transfer_searcher) = entity!("container_transfer");
    let (consumption_report_repo, consumption_report_searcher) = entity!("consumption_report");

    // Projections
    let (container_inventory_repo, container_inventory_searcher) = entity!("container_inventory");
    let (hen_productivity_repo, hen_productivity_searcher) = entity!("hen_productivity");
    let (farm_output_repo, farm_output_searcher) = entity!("farm_output");
